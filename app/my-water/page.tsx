...
export default function MyWaterPage() {
  ...
  const loadData = () => {
    ...
    const allWaterSources = Object.values(currentRegions).flatMap((region) => region.waterSources);
    const uniqueWaterSources = Array.from(new Map(allWaterSources.map((ws) => [ws.label + ws.id, ws])).values());
