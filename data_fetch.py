...
def get_bounding_box(lat: float, lon: float, offset: float=BBOX_OFFSET) -> Dict:
    return {'west': lon - offset, 'south': lat - offset, 'east': lon + offset, 'north': lat + offset, 'crs': 'EPSG:4326'}
...
def _build_composite(conn: openeo.Connection, bbox: Dict, start_date: str, end_date: str, band: str, use_scl_mask: bool=True) -> openeo.DataCube:
    bands_to_load = [band, 'SCL'] if use_scl_mask else [band]
    cube = conn.load_collection(COLLECTION, spatial_extent=bbox, temporal_extent=[start_date, end_date], bands=bands_to_load, max_cloud_cover=90)
    if use_scl_mask:
        scl = cube.band('SCL')
        valid_mask = (scl == 3) | (scl == 4) | (scl == 5) | (scl == 6) | (scl == 7) | (scl == 8)
...
def fetch_era5_rainfall(conn: openeo.Connection, bbox: Dict, days_back: int=RAINFALL_DAYS_BACK) -> Optional[float]:
...
def fetch_rainfall_forecast(lat: float, lon: float, days: int=FORECAST_DAYS) -> List[Dict[str, Any]]:
    today = datetime.utcnow().date()
    end_date = today + timedelta(days=days)
    url = f'https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&daily=precipitation_sum&forecast_days={days + 1}&timezone=UTC'
    ...
    try:
        ...
        for date_str, val in zip(dates, values):
            ...
            if date_str > end_date.strftime('%Y-%m-%d'):
                break
            forecast.append({'date': date_str, 'predicted_rainfall_mm': round(val, 2) if val is not None else None})
...
def fetch_sentinel2_bands(lat: float, lon: float) -> Dict[str, Any]:
    ...
    attempts = [(15, True, '15-day window + SCL mask'), (15, False, '15-day window, NO mask (fallback)'), (45, False, '45-day window, NO mask (wide fallback)')]
    for days_back, use_mask, label in attempts:
        ...
        if nan_frac < NAN_THRESHOLD:
            ...
            return {'bands': bands, 'bbox': bbox, 'time_range': (start_date, end_date), 'timestamp': datetime.utcnow().isoformat(), 'masked': use_mask, 'rainfall_mm': rainfall_mm, 'rainfall_forecast': rainfall_forecast}
        logger.warning(f"Strategy '{label}': NaN={nan_frac:.1%} exceeds threshold. Trying next.")
    raise ValueError(f'No valid Sentinel-2 data for lat={lat}, lon={lon} after all fallback strategies.')