    ...
    const popupContent = () => {
        ...
        if (source.satelliteStatus === "unavailable") {
            return (
                <div className="p-3 min-w-[210px]">
                    <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-1">{source.label}</h3>
                    <p className="text-xs text-amber-600 dark:text-amber-400 mb-2">Saved offline — no satellite data yet</p>
                    <Link
                        href={`/water-source/${source.id}`}
                        className="block text-center rounded-md bg-[#0277bd] py-1.5 text-xs font-semibold text-white hover:bg-[#01579b] transition-colors"
                    >
...
function MapPage({
    ...
    const handleSavePoint = async (e: React.FormEvent) => {
        ...
        analyzeWater(lat, lng)
            ...
            .catch(() => markWaterSourceSatelliteStatus(id, "error"));
    };
                    ...
                    waterSources.map((source) => {
                        ...
                        return (
                            <div key={source.id} className="flex flex-col gap-1.5 border-b border-slate-100 dark:border-slate-700 pb-2.5 last:border-0 last:pb-0">
                                <div className="flex items-center gap-1.5">
                                    <div className={cn("h-2 w-2 shrink-0 rounded-full", source.pending && "animate-pulse")} style={{ backgroundColor: color }} />
                                    <div className="text-xs font-semibold text-slate-900 dark:text-slate-100">{source.label}</div>
                                </div>
                                <div className="text-sm font-bold" style={{ color }}>
                                    {source.pending ? "Analyzing…" : getDisturbanceLabel(source)}
                                </div>
                                {!source.pending && source.disturbances.length > 0 && (
